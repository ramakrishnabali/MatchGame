...
import ThumbnailImage from '../ThumbnailImages'
...
class FruitsMatchGame extends Component {
  state = {
    category: tabsList[0].tabId,
    matchUrl: imagesList[0].imageUrl,
    time: 60,
    count: 0,
    isSame: false,
  }
  ...
  render() {
    const {matchUrl, category, time, count, isSame} = this.state

    const updatedThumbnails = imagesList.filter(
      ...
      <div className="appContainer">
        <nav className="navbar">
          ...
          <ul className="scoreCard">
            <p className="score">
              Score: <span className="timer">{count}</span>
            </p>
        ...
        {isSame === true ? (
          <ul className="gameCard">
            ...
            <p className="gameScore">{count}</p>
            <li className="playAgainContainer">
              <img
          ...
          <>
            <div className="matchContainer">
              <img src={matchUrl} alt="match" className="matchImage" />
            </div>

            <ul className="tabContainer">
              {tabsList.map(eachTab => (
                <Tabs
                  changeFunc={this.changeCategory}
                  tab={eachTab}
                  key={eachTab.id}
                />
              ))}
            ...
            <ul className="thumbnailContainer">
              {updatedThumbnails.map(eachImage => (
                <ThumbnailImage
                  isSameImage={this.matchingImage}
                  thumbnail={eachImage}
                  key={eachImage.id}
                />
              ))}
...
export default FruitsMatchGame
